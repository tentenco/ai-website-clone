...
function runValidation(value, schema, rootSchema, path, errors) {
  ...
  if (schema.enum && !schema.enum.some((entry) => deepEqual(value, entry))) {
    errors.push({
      path,
      message: `must be one of ${schema.enum.map(JSON.stringify).join(", ")}`,
    });
  }
  ...
  for (const keyword of ["allOf", "anyOf", "oneOf"]) {
    ...
    const passing = branchResults.filter(
    ...
    if (schema.uniqueItems) {
      for (let index = 0; index < value.length; index += 1) {
        if (value.slice(0, index).some((entry) => deepEqual(entry, value[index]))) {
          errors.push({
            path: `${path}/${index}`,
            message: "must be unique in this array",
          });
        }
      }
    }
  ...
  if (value !== null && typeof value === "object" && !Array.isArray(value)) {
    ...
    for (const required of schema.required ?? []) {
      if (!Object.hasOwn(value, required)) {
        errors.push({
          path,
          message: `missing required property ${JSON.stringify(required)}`,
        });
      }
    }
